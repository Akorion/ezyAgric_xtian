    ...
    <label for="inputPassword3" class="control-label">District</label>
     <div>
        ...
        <?php if(isset($_GET['token'])&&$_GET['token']!=""&&isset($_GET['type'])&&$_GET['type']!=""){
      	 ...
      	 $table="dataset_".$id;
      	 $row0s= $mCrudFunctions->fetch_rows($table," DISTINCT TRIM(TRAILING '.' FROM va_location_va_district)  as va_location_va_district"," 1 ORDER BY va_location_va_district ASC");
      	 $temp_array= array();
      	 foreach($row0s as $row){
      	 $value=$util_obj->captalizeEachWord($row['va_location_va_district']);
...
<div class="col-sm-2">
    ...
    <div class="row" id="age" >
     <div class="col-sm-2">
       <input onblur="setminrange()" id="age_min" type="number" class="form-control" name="min" placeholder="Min" style="background-color:#fff !important;
       border: 1px solid #d2d6de;border-radius: 4px;height:2em;">
     </div>
     <div class="col-sm-2" style="margin-left: 4em;">
       <input  onblur="setmaxrange()" id="age_max" type="number"  name="max" class="form-control" placeholder="Max" style="background-color:#fff !important;
...
<div class="col-sm-2">
    <label style=" padding-top:10px;"  class="control-label">Age</label>
       <input oninput="setage()" onblur="setage()"  id="age_" type="number" class="form-control" name="min" placeholder="Age" style="background-color: #fff; border: 1px solid #d2d6de; border-radius: 4px;height:2em;">
</div>
