...
/////////////////////////get farmer datasets
    if (isset($_POST['datasets']) && isset($_SESSION["client_id"])) {
        ...
        foreach ($row0s as $row) {
            ...
            elseif($role == 2) {
                $tb_rows = $mCrudFunctions->fetch_rows("dataset_".$row['id'], "DISTINCT(biodata_cooperative_name) AS cooperative", "sacco_branch_name LIKE '$branch'");
                foreach ($tb_rows as $tb){
                    $label = $util_obj->captalizeEachWord($tb['cooperative']);
                    $lable = str_replace(".", "", $label);
                    echo "<option value=\"$Id\">$lable</option>";
                }
            }
            else {
                $tb_rows = $mCrudFunctions->fetch_rows("dataset_".$row['id'], "DISTINCT(biodata_cooperative_name) AS cooperative", "biodata_cooperative_name LIKE '$branch'");
                foreach ($tb_rows as $tb){
                    $label = $util_obj->captalizeEachWord($tb['cooperative']);
                    $lable = str_replace(".", "", $label);
                    echo "<option value=\"$Id\">$lable</option>";
...
///////////////////////////get districts
    if (isset($_POST['district']) && isset($_SESSION["client_id"])) {
        ...
        elseif ($role == 2){
            $row0s = $mCrudFunctions->fetch_rows($table, " DISTINCT TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district)  as biodata_farmer_location_farmer_district", " sacco_branch_name LIKE '$branch' ORDER BY biodata_farmer_location_farmer_district ASC ");
            echo "<option value=\"all\">all</option>";
            $temp_array = array();
            foreach ($row0s as $row) {
                $value = $util_obj->captalizeEachWord($row['biodata_farmer_location_farmer_district']);
            ...
            $row0s = $mCrudFunctions->fetch_rows($table, " DISTINCT TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district)  as biodata_farmer_location_farmer_district", " biodata_cooperative_name LIKE '$branch' ORDER BY biodata_farmer_location_farmer_district ASC ");
            echo "<option value=\"all\">all</option>";
            $temp_array = array();
            foreach ($row0s as $row) {
                $value = $util_obj->captalizeEachWord($row['biodata_farmer_location_farmer_district']);
...
/////////////////////////////get subcountry ///////
    if (isset($_POST['district_subcounty']) && isset($_POST['id'])) {
        ...
        elseif ($role == 2){
            $row0s = $mCrudFunctions->fetch_rows($table, " DISTINCT  TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_subcounty) as biodata_farmer_location_farmer_subcounty ", "  TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district)  LIKE '$district' AND sacco_branch_name LIKE '$branch' ORDER BY biodata_farmer_location_farmer_subcounty ASC ");
            echo "<option value=\"all\">all</option>";
            $temp_array = array();
            foreach ($row0s as $row) {
                $value = $util_obj->captalizeEachWord($row['biodata_farmer_location_farmer_subcounty']);
            ...
            $row0s = $mCrudFunctions->fetch_rows($table, " DISTINCT  TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_subcounty) as biodata_farmer_location_farmer_subcounty ", "  TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district)  LIKE '$district' AND biodata_cooperative_name LIKE '$branch' ORDER BY biodata_farmer_location_farmer_subcounty ASC ");
            echo "<option value=\"all\">all</option>";
            $temp_array = array();
            foreach ($row0s as $row) {
                $value = $util_obj->captalizeEachWord($row['biodata_farmer_location_farmer_subcounty']);
...
//get parishes for county
    if (isset($_POST['parish_district']) && isset($_POST['parish_subcounty']) && isset($_POST['id'])) {
        ...
        elseif ($role == 2){
            $row0s = $mCrudFunctions->fetch_rows($table, " DISTINCT TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_parish) as biodata_farmer_location_farmer_parish  ", " TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district)  LIKE '$parish_district' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_subcounty)  LIKE '$county' AND sacco_branch_name LIKE '$branch' ORDER BY biodata_farmer_location_farmer_parish ASC");
            echo "<option value=\"all\">all</option>";
            $temp_array = array();
            foreach ($row0s as $row) {
                $value = $util_obj->captalizeEachWord($row['biodata_farmer_location_farmer_parish']);
            ...
            $row0s = $mCrudFunctions->fetch_rows($table, " DISTINCT TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_parish) as biodata_farmer_location_farmer_parish  ", " TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district)  LIKE '$parish_district' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_subcounty)  LIKE '$county' AND biodata_cooperative_name LIKE '$branch' ORDER BY biodata_farmer_location_farmer_parish ASC");
            echo "<option value=\"all\">all</option>";
            $temp_array = array();
            foreach ($row0s as $row) {
                $value = $util_obj->captalizeEachWord($row['biodata_farmer_location_farmer_parish']);
...
//get villages from parishes
    if (isset($_POST['village_district']) && isset($_POST['village_subcounty']) && isset($_POST['village_parish']) && isset($_POST['id'])) {
        ...
        elseif ($role == 2){
            $row0s = $mCrudFunctions->fetch_rows($table, " DISTINCT TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_village) as biodata_farmer_location_farmer_village ", " TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district)  LIKE '$village_district' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_subcounty)  LIKE '$village_subcounty' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_parish)  LIKE '$parish' AND sacco_branch_name LIKE '$branch' ORDER BY biodata_farmer_location_farmer_village ASC ");
            echo "<option value=\"all\">all</option>";
            $temp_array = array();
            foreach ($row0s as $row) {
                $value = $util_obj->captalizeEachWord($row['biodata_farmer_location_farmer_village']);
            ...
            $row0s = $mCrudFunctions->fetch_rows($table, " DISTINCT TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_village) as biodata_farmer_location_farmer_village ", " TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district)  LIKE '$village_district' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_subcounty)  LIKE '$village_subcounty' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_parish)  LIKE '$parish' AND biodata_cooperative_name LIKE '$branch' ORDER BY biodata_farmer_location_farmer_village ASC ");
            echo "<option value=\"all\">all</option>";
            $temp_array = array();
            foreach ($row0s as $row) {
                $value = $util_obj->captalizeEachWord($row['biodata_farmer_location_farmer_village']);
