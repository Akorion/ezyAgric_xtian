...
<tbody >";
  ...
  COUNT(DISTINCT(biodata_farmer_location_farmer_district))  as frequency ", "biodata_cooperative_name LIKE '$branch' GROUP BY district ORDER BY frequency Desc ");
                        ...
                        $total_males = $mCrudFunctions->get_count($table, " lower(biodata_gender) = 'male' AND biodata_cooperative_name LIKE '$branch' ");
                        $total_females = $mCrudFunctions->get_count($table, " lower(biodata_gender) = 'female' AND biodata_cooperative_name LIKE '$branch' ");
                        $ttl = $total_males + $total_females;
                        $gender_males = round(($total_males / $ttl) * 100, 1);
                        $gender_females = round(($total_females / $ttl) * 100, 1);
                        ...
                        if ($gender == 'male') {
                            $gender_males = $mCrudFunctions->get_count($table, " lower(biodata_gender) = 'male' AND biodata_cooperative_name LIKE '$branch' ");
                            $gender_females = 0;
                        } else {
                            $gender_males = 0;
                            $gender_females = $mCrudFunctions->get_count($table, " lower(biodata_gender) = 'female' AND biodata_cooperative_name LIKE '$branch' ");
                        }
...
///////////////////////////////////////////subcounties
  ...
  COUNT(DISTINCT(biodata_farmer_location_farmer_subcounty)) as frequency ", "  TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district) LIKE '$district' AND biodata_cooperative_name LIKE '$branch' GROUP BY subcounty  ORDER BY frequency Desc ");
                            ...
                            $total_males = $mCrudFunctions->get_count($table, "   TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district) LIKE '$district'  AND lower(biodata_gender) = 'male' AND biodata_cooperative_name LIKE '$branch'");
                            $total_females = $mCrudFunctions->get_count($table, " TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district) LIKE '$district'  AND lower(biodata_gender) = 'female' AND biodata_cooperative_name LIKE '$branch'");
                            $ttl = $total_males + $total_females;
                            $gender_males = round(($total_males / $ttl) * 100, 1);
                            $gender_females = round(($total_females / $ttl) * 100, 1);
                            ...
                            if ($gender == 'male') {
                                $gender_males = $mCrudFunctions->get_count($table, "   TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district) LIKE '$district'  AND lower(biodata_gender) = 'male' AND biodata_cooperative_name LIKE '$branch'");
                                $gender_females = 0;
                            } else {
                                $gender_males = 0;
                                $gender_females = $mCrudFunctions->get_count($table, " TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district) LIKE '$district'  AND lower(biodata_gender) = 'female' AND biodata_cooperative_name LIKE '$branch'");
                            }
                            $rows = $mCrudFunctions->fetch_rows($table, "TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_subcounty)  as subcounty, 
...
///////////////////////////////////////////parish
  ...
  COUNT(DISTINCT(biodata_farmer_location_farmer_parish)) as frequency ", " TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district) LIKE '$district' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_subcounty) LIKE '$subcounty' AND biodata_cooperative_name LIKE '$branch' GROUP BY parish ORDER BY frequency Desc ");
                                $total_males = $mCrudFunctions->get_count($table, "TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district) LIKE '$district' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_subcounty) LIKE '$subcounty' AND lower(biodata_gender) = 'male' AND biodata_cooperative_name LIKE '$branch' ");
                                $total_females = $mCrudFunctions->get_count($table, " TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district) LIKE '$district' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_subcounty) LIKE '$subcounty' AND lower(biodata_gender) = 'female' AND biodata_cooperative_name LIKE '$branch' ");
                                $ttl = $total_males + $total_females;
                                $gender_males = round(($total_males / $ttl) * 100, 1);
                                $gender_females = round(($total_females / $ttl) * 100, 1);
                            } else {
                                ...
                                if ($gender == 'male') {
                                    $gender_males = $mCrudFunctions->get_count($table, "TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district) LIKE '$district' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_subcounty) LIKE '$subcounty' AND lower(biodata_gender) = 'male' AND biodata_cooperative_name LIKE '$branch' ");
                                    $gender_females = 0;
                                } else {
                                    $gender_males = 0;
                                    $gender_females = $mCrudFunctions->get_count($table, " TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district) LIKE '$district' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_subcounty) LIKE '$subcounty' AND lower(biodata_gender) = 'female' AND biodata_cooperative_name LIKE '$branch' ");
...
///////////////////////////////////////////village
  ...
  COUNT(DISTINCT(biodata_farmer_location_farmer_village)) as frequency ", " TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district) LIKE '$district' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_subcounty) LIKE '$subcounty' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_parish) LIKE '$parish' AND biodata_cooperative_name LIKE '$branch'  GROUP BY village ORDER BY frequency Desc ");
                                    ...
                                    $gender = $_POST['gender'];
                                    if ($gender == 'male') {
                                        $gender_males = $mCrudFunctions->get_count($table, "   TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district) LIKE '$district' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_subcounty) LIKE '$subcounty' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_parish) LIKE '$parish'  AND lower(biodata_gender) = 'male' AND biodata_cooperative_name LIKE '$branch' ");
                                        $gender_females = 0;
                                    } else {
                                        $gender_males = 0;
                                        $gender_females = $mCrudFunctions->get_count($table, "   TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district) LIKE '$district' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_subcounty) LIKE '$subcounty' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_parish) LIKE '$parish'  AND lower(biodata_gender) = 'female' AND biodata_cooperative_name LIKE '$branch' ");
