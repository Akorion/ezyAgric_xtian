...
if (isset($_POST['district']) && isset($_POST['id'])) {
    ...
    if ($dataset_type == "Farmer") {
        ...
        $row0s = $mCrudFunctions->fetch_rows($table, " DISTINCT TRIM(TRAILING '.' FROM va_location_va_subcounty) as va_location_va_subcounty ", " TRIM(TRAILING '.' FROM va_location_va_district) LIKE '$district' ORDER BY va_location_va_subcounty ASC");
        echo "<option value=\"all\">All</option>";
        //print_r($district." ".$table." ".$_POST['id']);
        $temp_array = array();
        foreach ($row0s as $row) {
            $value = $util_obj->captalizeEachWord($row['va_location_va_subcounty']);
            $value = str_replace(".", "", $value);
            // echo "<option value=\"$value\">$value</option>";
            if (in_array($value, $temp_array)) {
            } else {
...
if (isset($_POST['parish_district']) && isset($_POST['parish_subcounty']) && isset($_POST['id'])) {
    ...
    if ($dataset_type == "Farmer") {
        ...
        $row0s = $mCrudFunctions->fetch_rows($table, " DISTINCT TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_parish) as biodata_farmer_location_farmer_parish ", " TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_district) LIKE '$district' AND TRIM(TRAILING '.' FROM biodata_farmer_location_farmer_subcounty) LIKE '$subcounty' ORDER BY biodata_farmer_location_farmer_parish ASC ");
        echo "<option value=\"all\">all</option>";
        $temp_array = array();
        foreach ($row0s as $row) {
